...
public class BigQueryAvroUtils {
	...
	public static class BigQueryAvroMapper {
		...
		private static final ImmutableMultimap<String, Type> BIG_QUERY_TO_AVRO_TYPES = ImmutableMultimap
				.<String, Type>builder().put("STRING", Type.STRING).put("GEOGRAPHY", Type.STRING)
				.put("BYTES", Type.BYTES).put("INTEGER", Type.INT).put("FLOAT", Type.FLOAT).put("FLOAT64", Type.DOUBLE)
				.put("NUMERIC", Type.BYTES).put("BOOLEAN", Type.BOOLEAN).put("INT64", Type.LONG).put("INT32", Type.INT)
				.put("TIMESTAMP", Type.LONG).put("RECORD", Type.RECORD).put("DATE", Type.INT)
				.put("DATETIME", Type.STRING).put("TIME", Type.LONG).put("STRUCT", Type.RECORD).put("ARRAY", Type.ARRAY)
				.build();

		private static final ImmutableMultimap<Type, String> AVRO_TYPES_TO_BIG_QUERY = BIG_QUERY_TO_AVRO_TYPES
				.inverse();
	 ...
	 * and BigQuery uses the proleptic Gregorian calendar.
	 */
	...
	private static final java.time.format.DateTimeFormatter ISO_LOCAL_TIME_FORMATTER_MICROS = new DateTimeFormatterBuilder()
			...
			.appendValue(SECOND_OF_MINUTE, 2).appendLiteral('.').appendFraction(NANO_OF_SECOND, 6, 6, false)
			.toFormatter();

	private static final java.time.format.DateTimeFormatter ISO_LOCAL_TIME_FORMATTER_MILLIS = new DateTimeFormatterBuilder()
			.appendValue(HOUR_OF_DAY, 2).appendLiteral(':').appendValue(MINUTE_OF_HOUR, 2).appendLiteral(':')
			.appendValue(SECOND_OF_MINUTE, 2).appendLiteral('.').appendFraction(NANO_OF_SECOND, 3, 3, false)
			.toFormatter();
	 ...
	 * used by JSON export. Time records are stored in "microseconds since midnight"
	 * format.
	 */
	...
	/**
	 * Utility function to convert from an Avro {@link GenericRecord} to a BigQuery
	 * {@link TableRow}.
	 *
	 * <p>
	 * See <a href=
	 * "https://cloud.google.com/bigquery/exporting-data-from-bigquery#config">"Avro
	...
	private static TableRow convertGenericRecordToTableRow(GenericRecord record, List<TableFieldSchema> fields) {
		...
		for (TableFieldSchema subSchema : fields) {
			// Per https://cloud.google.com/bigquery/docs/reference/v2/tables#schema, the
			// name field
			...
			if (convertedValue != null) {
				// To match the JSON files exported by BigQuery, do not include null values in
				// the output.
	...
	private static Object convertRequiredField(Type avroType, LogicalType avroLogicalType, TableFieldSchema fieldSchema,
			Object v) {
		// REQUIRED fields are represented as the corresponding Avro types. For example,
		// a BigQuery
		...
		/*
		 * verify( expectedAvroTypes.contains(avroType),
		 * "Expected Avro schema types %s for BigQuery %s field %s, but received %s",
		 * expectedAvroTypes, bqType, fieldSchema.getName(), avroType);
		 */
		// For historical reasons, don't validate avroLogicalType except for with
		// NUMERIC.
		// BigQuery represents NUMERIC in Avro format as BYTES with a DECIMAL logical
		// type.
		switch (bqType) {
		case "STRING":
		case "DATETIME":
		case "GEOGRAPHY":
			// Avro will use a CharSequence to represent String objects, but it may not
			// always use
		...
		case "DATE":
			if (avroType == Type.INT) {
				verify(v instanceof LocalDate, "Expected org.joda.time.LocalDate, got %s", v.getClass());
		...
		case "TIME":
			if (avroType == Type.LONG) {
				...
				verify(avroLogicalType instanceof LogicalTypes.TimeMicros, "Expected TimeMicros logical type");
		...
		case "FLOAT":
			...
			return v;
		case "NUMERIC":
			// NUMERIC data types are represented as BYTES with the DECIMAL logical type.
			// They are
			// converted back to Strings with precision and scale determined by the logical
			// type.
			...
			BigDecimal numericValue = new Conversions.DecimalConversion().fromBytes((ByteBuffer) v,
					Schema.create(avroType), avroLogicalType);
		...
		default:
			throw new UnsupportedOperationException(
					String.format("Unexpected BigQuery field schema type %s for field named %s", fieldSchema.getType(),
							fieldSchema.getName()));
		}
	}
	...
	static Schema toGenericAvroSchema(String schemaName, List<TableFieldSchema> fieldSchemas) {
		...
		return Schema.createRecord(schemaName, "org.apache.beam.sdk.io.gcp.bigquery",
				"Translated Avro Schema for " + schemaName, false, avroFields);
	}
	...
	private static Field convertField(TableFieldSchema bigQueryField) {
		...
		return new Field(bigQueryField.getName(), fieldSchema, bigQueryField.getDescription(),
		...
		if (schema != null) {
			for (Field field : schema.getFields()) {
				...
				if ("ARRAY".equals(type)) {
					...
					if (childSchema.getType() == Type.RECORD) {
						List<TableFieldSchema> child = getTableFieldSchema(field.schema().getElementType());
						TableFieldSchema tfs = new TableFieldSchema().setName(field.name()).setType("STRUCT")
								.setFields(child).setMode("REPEATED");
						tableFieldSchemas.add(tfs);
					} else {

						TableFieldSchema tfs = new TableFieldSchema().setName(field.name())
								.setType(getBqType(childSchema.getFields().get(0))).setMode("REPEATED");
						tableFieldSchemas.add(tfs);
					}
	...
	static String getBqType(Field field) {
		...
		if (t.getType().equals(Type.LONG) && "timestamp-millis".equals(logicalType)) {
			return "TIMESTAMP";
		} else if (t.getType().equals(Type.INT) && "date".equals(logicalType)) {
			return "DATE";
		} else {
			return BigQueryAvroMapper.getBigQueryTypes(t.getType());
		}
	}
}
